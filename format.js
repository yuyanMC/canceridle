...
function formatValue(notation, value, places, placesUnder1000) {
    ...
    if ((value <= Number.MAX_VALUE || (player.break && (player.currentChallenge == "" || !new Decimal(Number.MAX_VALUE).equals(player.challengeTarget)) )) && (value >= 1000)) {
        if (value instanceof Decimal) {
           var power = value.e
           var matissa = value.div(Decimal.pow(10,power-5)).floor().div(100000)
        } else {
...
shorten = function (money) {
  return formatValue("Mixed scientific", money, 2, 2);
};

shortenCosts = function (money) {
  return formatValue("Mixed scientific", money, 0, 0);
};

shortenDimensions = function (money) {
  return formatValue("Mixed scientific", money, 2, 0);
};

shortenMoney = function (money) {
  return formatValue("Mixed scientific", money, 2, 1);
};
